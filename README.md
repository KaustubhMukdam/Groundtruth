...
|-------|-----------|-----|
| LLM | IBM Granite (`granite-4-h-small`) | Available on free tier (Dallas/US-South) + excellent at structured instruction-following; reliable for self-critique |
...
- **IBM Granite**, `ibm/granite-4-h-small` via watsonx.ai (Dallas/US-South). Used in 3 places: tool selection, explanation generation, and self-critique. Model ID confirmed via watsonx.ai supported models API.
